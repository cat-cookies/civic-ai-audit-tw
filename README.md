...
# v4.0.0：學術研究流程、節能 AI 與三版本修法

v4 將搜尋、研究與修法重新分層：

- 首頁示例改為一般民眾常見問題，不讀取使用者過去資料產生示例。
- 搜尋採詞彙證據與文件頻率排序，只自動展開正式名稱與常用縮寫。
- AI 分為節能、標準、高風險三種資源模式；規則式工作不使用模型。
- 研究輸出固定包含主張—證據矩陣、法律／政策分流、反方、方法、跨國比較與限制。
- 修法頁提供 A 最小修正、B 權衡修正、C 制度性修正三版，並可下載 `.xlsx`。
- `backend/hf-space` 可部署為 Hugging Face Docker Space，集中保存免費模型供應商金鑰。
- 設計自我批判與殘餘風險見 `docs/SELF_CRITIQUE_V4.md`。
