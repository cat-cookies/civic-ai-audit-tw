...
class V3FeatureTest(unittest.TestCase):
    ...
    def test_public_site_contains_new_data(self):
        ...
        ai=(ROOT/'_site/ai-engine.js').read_text(encoding='utf-8')
        self.assertIn('CivicWorkflow', app)
        self.assertIn('fetchEligibleModels', ai)
        self.assertIn('OpenRouter', ai)
        self.assertTrue((ROOT/'_site/workflow.js').exists())
