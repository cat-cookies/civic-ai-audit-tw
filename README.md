...
- 收錄三黨中央黨部官網、Facebook、Instagram、Threads 官方入口與資料取得限制。
- 黨綱／黨章只萃取可區辨的意識形態命題，降低「為民服務」等共通口號權重。
- 政黨跨期分析必須對齊議題、日期、主體層級、文類、原文及官方來源。
- 媒體單篇分析顯示各黨批評、稱讚、描述性表述、官方論述接近度與語句證據。
- 媒體整體偏向至少需要30篇、3個議題、4週及人工抽樣複核。
- 政府資金、持股或任命只標示結構性依賴風險，不直接推論親執政者。
- 跨國文獻均附完整 citation、可移植條件與中華民國適用性評估。


## v6.1.0 網域感知即時搜尋

- 首頁每次查詢先進行線上搜尋，本地索引降級為可折疊的補充資料。
- 使用者可選擇法律、政策、醫療、學術、統計、預算、政黨或媒體領域。
- 可選法域／國家，或只搜尋使用者指定的完整網域。
- 學術與醫療可直接使用 Crossref／Europe PMC；一般網頁搜尋由受控後端使用核准搜尋API。
